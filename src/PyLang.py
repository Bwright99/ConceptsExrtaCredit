...
class Lexer:
  ...
  def createKEYWORDS(self):
    ...
    while self.current_char != None:
      if self.current_char in ' \t':
        ...
        KEYWORDS.append(Token(NEWLINE, pos_start=self.pos))
        self.advance()
      elif self.current_char in DIGITS:
        KEYWORDS.append(self.createDigit())
      elif self.current_char in LETTERS:
        KEYWORDS.append(self.make_identifier())
      elif self.current_char == '"':
        KEYWORDS.append(self.make_string())
      elif self.current_char == '+':
        KEYWORDS.append(Token(PLUS, pos_start=self.pos))
        self.advance()
      elif self.current_char == '-':
        KEYWORDS.append(self.make_minus_or_arrow())
      elif self.current_char == '*':
        KEYWORDS.append(Token(MULTIPLY, pos_start=self.pos))
        self.advance()
      elif self.current_char == '/':
        KEYWORDS.append(Token(DIVIDE, pos_start=self.pos))
        self.advance()
      elif self.current_char == '^':
        KEYWORDS.append(Token(CARAT, pos_start=self.pos))
        self.advance()
      elif self.current_char == '(':
        KEYWORDS.append(Token(LPAREN, pos_start=self.pos))
        self.advance()
      elif self.current_char == ')':
        KEYWORDS.append(Token(RPAREN, pos_start=self.pos))
        self.advance()
      elif self.current_char == '[':
        KEYWORDS.append(Token(LBRACE, pos_start=self.pos))
        self.advance()
      elif self.current_char == ']':
        KEYWORDS.append(Token(RBRACE, pos_start=self.pos))
        self.advance()
      elif self.current_char == '!':
        ...
        KEYWORDS.append(self.make_equals())
      elif self.current_char == '<':
        KEYWORDS.append(self.make_less_than())
      elif self.current_char == '>':
        KEYWORDS.append(self.make_greater_than())
      elif self.current_char == ',':
        KEYWORDS.append(Token(COMMA, pos_start=self.pos))
        self.advance()
      else:
    ...
    KEYWORDS.append(Token(EOF, pos_start=self.pos))
  ...
  def make_identifier(self):
    ...
    tok_type = KEYWORD if id_str in KEYWORDS else ID
...
class Parser:
  ...
  def update_current_tok(self):
    if self.tok_idx >= 0 and self.tok_idx < len(self.KEYWORDS):
      self.current_tok = self.KEYWORDS[self.tok_idx]
  ...
  def parse(self):
    ...
    if not res.error and self.current_tok.type != EOF:
      return res.failure(InvalidSyntaxError(
        ...
        "Token cannot appear after previous KEYWORDS"
      ))
    return res
  ...
  def CARATer(self):
    return self.bin_op(self.call, (CARAT, ), self.factor)
...
class Interpreter:
  ...
  def visit_BinOpNode(self, node, context):
    ...
    elif node.op_tok.type == CARAT:
      result, error = left.CARATed_by(right)
...
def run(fn, text):
  lexer = Lexer(fn, text)
  KEYWORDS, error = lexer.createKEYWORDS()
